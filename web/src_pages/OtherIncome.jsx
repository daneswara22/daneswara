...
export default function OtherIncome() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await api.post("/other-income", { category: form.category, amount: Number(form.amount), note: form.note, date: form.date, source: form.source });
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody>
            ...
            {filtered.length === 0 && <tr><td colSpan={6} className="px-4 py-10 text-center text-muted-foreground">{term ? "Tidak ada pendapatan cocok." : "Belum ada pendapatan lain-lain."}</td></tr>}
          </tbody>
        </table>
      </div>
        ...
        <DialogContent data-testid="other-income-dialog">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <Select data-testid="other-income-source-select-1" value={form.source} onValueChange={(v) => setForm({ ...form, source: v })}>
                <SelectTrigger data-testid="other-income-source-select"><SelectValue placeholder="Pilih sumber dana" /></SelectTrigger>
                <SelectContent>
                  {FUND_SOURCES.map((s) => <SelectItem data-testid={`other-income-source-item-${s}`} key={s} value={s}>{s}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
