...
export const POST = handle(async (req: NextRequest) => {
  ...
  for (let idx = 0; idx < data.ids.length; idx++) {
    await prisma.products.updateMany({ where: { id: data.ids[idx], tenant_id: user.tenant_id }, data: { sort_order: idx } });
  }
  await logActivity(user.tenant_id, user, 'Atur Urutan Produk', `${data.ids.length} produk diurutkan ulang`);
