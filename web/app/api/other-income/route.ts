...
import { serializeOtherIncome } from '@/lib/serializers';
...
export const POST = handle(async (req: NextRequest) => {
  ...
  const row = await prisma.other_income.create({
    data: {
      id: newId(), tenant_id: user.tenant_id, category: data.category, amount: data.amount,
      note: data.note || '', date: d, user_name: user.name || '',
      fund_source: data.fund_source || null,
      created_at: new Date(),
    },
  });
  await logActivity(user.tenant_id, user, 'Tambah Pendapatan Lain-lain', `${data.category} - ${data.amount}`);
  return serializeOtherIncome(row);
});
