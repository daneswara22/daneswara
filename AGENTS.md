...
Kalau pemilik ternyata belum mengirim alamat MariaDB publik (`HOST:6796`) dan yang ada cuma alamat internal Coolify, jangan buang waktu mencoba menyambung ke sana, sandbox tidak akan bisa menjangkaunya. Jalur cadangannya: pakai MariaDB lokal di sandbox lewat `scripts/sandbox-db.sh`. Script itu dijalankan sebagai program supervisor (`command=/bin/bash /app/scripts/sandbox-db.sh`), membaca sendiri user, password, dan nama database dari `DATABASE_URL` di `web/.env` (jadi tidak ada kredensial di repo), memasang `mariadb-server` bila layer apt sandbox terhapus, menyimpan datadir di `/root/.sandbox-mariadb` supaya tidak hilang saat pod restart, lalu menerapkan skema Prisma dan `web/scripts/seed-dummy-sandbox.ts` hanya kalau databasenya masih kosong. Seed dummy itu isinya kecil dan sengaja begitu, sekitar 5-10 baris per tabel inti (produk, kategori, pelanggan, supplier, galeri, penjualan, pesanan, pembelian, pengeluaran, pendapatan lain, order custom tees), cukup untuk menelusuri semua halaman tanpa menyentuh data produksi. Kalau nanti alamat publiknya datang, cukup ganti satu baris `DATABASE_URL` di `web/.env`; script di atas otomatis berhenti sendiri begitu host-nya bukan `127.0.0.1`. Satu hal lagi yang pernah menipu: kalau MariaDB sandbox dijalankan lepas dari supervisor, prosesnya bisa mati diam-diam dan gejalanya muncul sebagai error Prisma semacam "The table `custom_tee_orders` does not exist in the current database", jadi selalu titipkan ke supervisor.
